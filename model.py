...
class MNISTDiffusion(nn.Module):
    def __init__(self, image_size, in_channels, time_embedding_dim=256, timesteps=1000, base_dim=32,
        ...
        # self.betas = self._linear_variance_schedule(timesteps).to(device)
        # self.betas = self._exponential_variance_schedule(timesteps).to(device)
    ...
    def _linear_variance_schedule(self, timesteps, start=0.0001, end=0.02):
        # 生成从 start 到 end 线性变化的方差序列
        betas = torch.linspace(start, end, steps=timesteps, dtype=torch.float32)
        return betas

    def _exponential_variance_schedule(self, timesteps, beta_min=0.0001, beta_max=0.02):
        # 生成指数变化的方差序列
        t = torch.arange(1, timesteps + 1, dtype=torch.float32)  # 时间步从1到timesteps
        betas = beta_min * (beta_max / beta_min) ** ((t - 1) / (timesteps - 1))
        return betas
